...
void strPower(char solarPWR[], char hydroPWR[], char loadPWR[]){

    if(LOAD_ON_FLAG == false){
    // During low res logging we can't accurately calculate these so
    // set them to zero
        dtostrf( 0.0, 4, 2, solarPWR);
        dtostrf( 0.0, 4, 2, hydroPWR);
        dtostrf( 0.0, 4, 2, loadPWR);
    ...
    dtostrf( (ENERGY_SOLAR/timeElapsed)      , 4, 2, solarPWR);
    dtostrf( (ENERGY_HYDRO/timeElapsed)      , 4, 2, hydroPWR);
    dtostrf( (ENERGY_USED/timeElapsed)      , 4, 2, loadPWR);
...
void energyUpdate(){
    ...
    // Energy solar is just the power generated by the solar panel
    ENERGY_SOLAR += Powers[4]*(Powers[5])*(float(ENERGY_TIME_ELAPSED)/1000.0);
    // Energy hydro is the 'missing power'. battery - used by loads - generated by solar = hydro
    ENERGY_HYDRO += ((Powers[0]*Powers[1]) - (Powers[2]*Powers[3]) - (Powers[4]*Powers[5])) *(float(ENERGY_TIME_ELAPSED)/1000.0);
